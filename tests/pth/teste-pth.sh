...
for((k=1; k<=32; k*=2)): do
    ...
    for ((j=1; j<=10; j++)); do
        ...
        for ((i=16; i<=8192; i *= 2)); do
            full+=`./mandelbrot_pth -2.5 1.5 -2.0 2.0 $i $k`
            seahorse+=`./mandelbrot_pth -0.8 -0.7 0.05 0.15 $i $k`
            elephant+=`./mandelbrot_pth 0.175 0.375 -0.1 0.1 $i $k`
            triple_spiral+=`./mandelbrot_pth -0.188 -0.012 0.554 0.754 $i $k`
        done
